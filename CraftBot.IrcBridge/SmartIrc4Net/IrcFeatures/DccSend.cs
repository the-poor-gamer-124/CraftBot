...
namespace Meebey.SmartIrc4net
{
    ...
    public class DccSend : DccConnection
    {
        ...
        internal DccSend(IrcFeatures irc, string user, IPAddress externalIpAdress, Stream file, string filename, long filesize, DccSpeed speed, bool passive, Priority priority) : base()
        {
            ...
            if (passive)
            {
                irc.SendMessage(SendType.CtcpRequest, user, "DCC SEND \"" + filename + "\" " + this.HostToDccInt(externalIpAdress).ToString() + " 0 " + filesize + " " + session, priority);
            }
            else
            {
                ...
                irc.SendMessage(SendType.CtcpRequest, user, "DCC SEND \"" + filename + "\" " + this.HostToDccInt(externalIpAdress).ToString() + " " + LocalEndPoint.Port + " " + filesize, priority);
            }
        }
        ...
        internal DccSend(IrcFeatures irc, IPAddress externalIpAdress, CtcpEventArgs e) : base()
        {
            ...
            {
                long filesize = 0;
                bool okIP = long.TryParse(e.Data.MessageArray[3], out long ip);
                bool okPo = int.TryParse(e.Data.MessageArray[4], out int port);  // port 0 = passive
                if (e.Data.MessageArray.Length > 5)
                {
                    /*bool okFs*/
                    _ = long.TryParse(this.FilterMarker(e.Data.MessageArray[5]), out filesize);
                    _Filesize = filesize;
                    _Filename = e.Data.MessageArray[2].Trim(new char[] { '\"' });
                }
                if (okIP && okPo)
                {
                    RemoteEndPoint = new IPEndPoint(IPAddress.Parse(this.DccIntToHost(ip)), port);
                    this.DccSendRequestEvent(new DccSendRequestEventArgs(this, e.Data.MessageArray[2], filesize));
                    return;
                }
                else
                {
        ...
        internal override void InitWork(object stateInfo)
        {
            ...
            this.DccSendStartEvent(new DccEventArgs(this));
            int bytes;
            ...
            if (_DirectionUp)
            {
                do
                {
                    ...
                    if (bytes > 0)
                    {
                        this.DccSendSentBlockEvent(new DccSendEventArgs(this, _Buffer, bytes));
                        Console.Write(".");
                    }
                } while (bytes > 0);
            }
            else
            {
                while ((bytes = Connection.GetStream().Read(_Buffer, 0, _Buffer.Length)) > 0)
                {
                    ...
                    {
                        Connection.GetStream().Write(this.GetAck(this.SentBytes), 0, 4);
                    }

                    this.DccSendReceiveBlockEvent(new DccSendEventArgs(this, _Buffer, bytes));
                }
            }

            isValid = false;
            isConnected = false;
            Console.WriteLine("--> Filetrangsfer Endet / Bytes sent: " + this.SentBytes + " of " + _Filesize);
            this.DccSendStopEvent(new DccEventArgs(this));
        }
        ...
        public bool AcceptRequest(Stream file, long offset)
        {
            ...
            try
            {
                ...
                if (RemoteEndPoint.Port == 0)
                {
                    ...
                    DccServer.Start();
                    LocalEndPoint = (IPEndPoint)DccServer.LocalEndpoint;
                    Irc.SendMessage(SendType.CtcpRequest, User, "DCC SEND \"" + _Filename + "\" " + this.HostToDccInt(ExternalIPAdress).ToString() + " " + LocalEndPoint.Port + " " + _Filesize);
                }
                else
                {
                    if (offset == 0)
        ...
        internal bool TryResume(CtcpEventArgs e)
        {
            if (User == e.Data.Nick)
            {
                if ((e.Data.MessageArray.Length > 4) && (_Filename == e.Data.MessageArray[2].Trim(new char[] { '\"' })))
                {
                    long.TryParse(this.FilterMarker(e.Data.MessageArray[4]), out long offset);
                    if (_File.CanSeek)
                    {
                        if (e.Data.MessageArray.Length > 5)
                        {
                            Irc.SendMessage(SendType.CtcpRequest, e.Data.Nick, "DCC ACCEPT " + e.Data.MessageArray[2] + " " + e.Data.MessageArray[3] + " " + e.Data.MessageArray[4] + " " + this.FilterMarker(e.Data.MessageArray[5]));
                        }
                        else
                        {
                            Irc.SendMessage(SendType.CtcpRequest, e.Data.Nick, "DCC ACCEPT " + e.Data.MessageArray[2] + " " + e.Data.MessageArray[3] + " " + this.FilterMarker(e.Data.MessageArray[4]));
                        }
