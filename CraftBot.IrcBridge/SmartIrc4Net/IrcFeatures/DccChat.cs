...
namespace Meebey.SmartIrc4net
{
    ...
    public class DccChat : DccConnection
    {
        ...
        internal DccChat(IrcFeatures irc, string user, IPAddress externalIpAdress, bool passive, Priority priority) : base()
        {
            ...
            if (passive)
            {
                irc.SendMessage(SendType.CtcpRequest, user, "DCC CHAT chat " + this.HostToDccInt(externalIpAdress).ToString() + " 0 " + session, priority);
                this.Disconnect();
            }
            else
            {
                ...
                irc.SendMessage(SendType.CtcpRequest, user, "DCC CHAT chat " + this.HostToDccInt(externalIpAdress).ToString() + " " + LocalEndPoint.Port, priority);
            }
        }
        ...
        internal DccChat(IrcFeatures irc, IPAddress externalIpAdress, CtcpEventArgs e) : base()
        {
            ...
            if (e.Data.MessageArray.Length > 4)
            {
                ...
                bool okPo = int.TryParse(this.FilterMarker(e.Data.MessageArray[4]), out int port);  // port 0 = passive
                if ((e.Data.MessageArray[2] == "chat") && okIP && okPo)
                {
                    ...
                    this.DccChatRequestEvent(new DccEventArgs(this));
                    return;
                }
                else
                {
        ...
        internal override void InitWork(object stateInfo)
        {
            ...
            this.DccChatStartEvent(new DccEventArgs(this));
            ...
            {
                this.DccChatReceiveLineEvent(new DccChatEventArgs(this, line));
                this.Lines++;
            }
            isValid = false;
            isConnected = false;
            this.DccChatStopEvent(new DccEventArgs(this));
        }
        ...
        public bool AcceptRequest()
        {
            ...
            try
            {
                if (RemoteEndPoint.Port == 0)
                {
                    ...
                    DccServer.Start();
                    LocalEndPoint = (IPEndPoint)DccServer.LocalEndpoint;
                    Irc.SendMessage(SendType.CtcpRequest, User, "DCC CHAT chat " + this.HostToDccInt(ExternalIPAdress).ToString() + " " + LocalEndPoint.Port);
                }
                else
                {
        ...
        public void WriteLine(string message)
        {
            if (isConnected)
            {
                ...
                this.Lines++;
                this.DccChatSentLineEvent(new DccChatEventArgs(this, message));
            }
            else
            {
